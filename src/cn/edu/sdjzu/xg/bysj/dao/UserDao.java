...
public final class UserDao {
	...
	public boolean changePassword(Integer id,String passwoord)throws SQLException{
		Connection connection = JdbcHelper.getConn();
		//写sql语句
		...
		//为预编译参数赋值
		preparedStatement.setString(1,passwoord);
...
//		User user = new User(1,"st","st",new Date(),teacher);
//		System.out.println(UserDao.getInstance().add(user));
		//测试修改
//		User user1 = UserDao.getInstance().find(1);
...
//		System.out.println(user1);
		//测试删除
		//测试查找
//		User user2 = UserDao.getInstance().find(1);
//		System.out.println("按照id查询find"+user2);
//		User user3 = UserDao.getInstance().findByUsername("st");
//		System.out.println("按照用户名查找"+user3);
		//测试登录
		User user4 = UserDao.getInstance().login("st","st");
		System.out.println("这是错误的账号密码登录"+user4);
		User user5 = UserDao.getInstance().login("st","123");
